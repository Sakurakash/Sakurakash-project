$(function () {
    ...
    let curId = curURL.substring(curURL.lastIndexOf("?id=") + "?id=".length);
    ...
    DetailApis.getDjRadio(curId)
        .then(function (data) {
            ...
            // 填充头部数据
            let topHtml = template('topInfo', data.data);
            $(".main-top").html(topHtml);
            // 填充底部数据
            let bottomHtml = template('bottomInfo', data.data);
            $(".main-bottom .content-default").html(bottomHtml);
            // 填充底部头部的数据
            $(".bottom-header>.top>p:nth-of-type(2)>b").text(data.data.programCount);
            // 刷新滚动范围
            mainScroll.refresh();
        })
        .catch(function (err) {
    ...
    $(".bottom-header>.top>p").click(function () {
        ...
        $(".bottom-header>.top>.line").animate({left: spanLeft}, 200);
        ...
        DetailApis.getProgram(curId, asc)
            .then(function (data) {
                ...
                data.programs.forEach(function (obj) {
                    obj.createTime = dateFormat("yyyy-MM-dd", new Date(obj.createTime));
                    obj.listenerCount = formatNum(obj.listenerCount);
                    let timeObj = formatTime(obj.duration);
