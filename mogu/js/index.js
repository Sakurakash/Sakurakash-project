...
function initGoods() {
    ...
    let html3 = template('footer', obj3);
    let oMainIn = document.querySelector(".main-in");
    oMainIn.innerHTML = oMainIn.innerHTML + html;
    let oCategory = document.querySelector(".category");
    oCategory.innerHTML = html2;
    let oFooter = document.querySelector(".footer-in");
    oFooter.innerHTML = html3;
    // 4.初始化控制文字行数的代码
    initTextRow();
    // 5.进行瀑布流布局
    waterfall();
...
function initNav() {
    let oNav = document.querySelector(".nav2>.nav-bottom");
    let oNavHeight = oNav.offsetHeight;
    window.onscroll = function () {
        if (window.scrollY >= oNavHeight){
            oNav.style.top = 0;
        }else {
            oNav.style.top = -oNavHeight + "px";
        }
    };
}
let timerId = null;
...
function waterfall() {
    ...
    let oMainIn = document.querySelector(".main-in");
    let colsNum = Math.floor(oMainIn.offsetWidth / oItems[0].offsetWidth);
    //创建一个数组,储存第一行元素的高度;
    let rowHeight = [];
